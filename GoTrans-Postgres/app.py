...
# 2. BUNDLE CSS FUTURISTIK & ELEGAN (ASLI)
st.markdown("""
    <style>
    .stApp {
...
try:
    ...
    # Terapkan Saringan Lanjutan ke Dataframe
            ...
            with col_btn:
                try:
                    ...
                    st.download_button(
                        ...
                        file_name=f"Gotrans_Ekstrak_{val_bulan}_{start_date}_to_{end_date}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
                except Exception as ex:
                    st.error(f"Gagal memproses ekstrak Excel: {ex}")

            # --- GRAFIK TREN HARIAN ASLI ---
    ...
    # TAB 2: DATA RAW OPERASIONAL
    with tab_raw:
        st.dataframe(df_filtered, use_container_width=True)

except Exception as e:
    st.error(f"⚠️ Gagal memuat data dari tabel `{tabel_aktif}` di database: Pastikan data untuk Bulan {val_bulan} Tahun {val_tahun} sudah diunggah.\n\nDetail Error: {e}")
